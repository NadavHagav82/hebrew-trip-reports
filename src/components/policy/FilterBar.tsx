...
export function FilterBar({
  ...
  return (
    <div className="mt-5 space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-l from-primary/5 via-background to-primary/10 p-5 shadow-sm">
        ...
        <div className="relative flex items-center gap-3 mb-4">
          ...
          {activeFilterCount > 0 && (
            <Badge className="mr-auto bg-primary text-primary-foreground shadow-sm hover:bg-primary/90">
        ...
        <div className="relative flex flex-wrap gap-3 items-center">
          ...
          <div className="relative group">
            <div className="absolute inset-0 rounded-lg bg-primary/5 opacity-0 group-focus-within:opacity-100 transition-opacity -m-0.5" />
            ...
            <Input
              ...
              className="relative pr-10 w-[240px] bg-background border-border/60 shadow-sm focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/60"
            />
            {searchQuery && (
              <button
                onClick={() => onSearchChange('')}
                className="absolute left-2.5 top-1/2 -translate-y-1/2 p-1.5 bg-muted/80 hover:bg-destructive/20 hover:text-destructive rounded-md transition-all z-10"
              >
                <X className="w-3 h-3" />
              </button>
            )}
          ...
          {filters.map((filter) => {
            ...
            return (
              <Select key={filter.key} value={filter.value} onValueChange={filter.onChange}>
                <SelectTrigger 
                  className={`w-[145px] transition-all duration-200 shadow-sm ${
                    isActive 
                      ? 'bg-primary/10 border-primary/40 text-primary ring-2 ring-primary/20 font-medium' 
                ...
                <SelectContent className="bg-popover border border-border/60 shadow-xl rounded-xl overflow-hidden">
                  {filter.options.map((option) => (
                    <SelectItem 
                      key={option.value} 
                      value={option.value}
                      className="cursor-pointer hover:bg-primary/10 focus:bg-primary/10 transition-colors py-2.5"
                    >
          ...
          {hasActiveFilters && (
            <Button 
              ...
              className="border-destructive/30 text-destructive hover:bg-destructive hover:text-destructive-foreground hover:border-destructive transition-all duration-200 shadow-sm"
            >
